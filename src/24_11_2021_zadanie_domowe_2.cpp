...
public:
    Kierownik(double pensja, int iloscPodwladnych) : Pracownik(pensja), iloscPodwladnych(iloscPodwladnych) {}
    ~Kierownik() {}
    void przyznaj_bonus(double kwota)
    {
...
{
    Pracownik *rodokot[4];
    rodokot[0] = new Pracownik(2400);
    rodokot[1] = new Pracownik(2600);
    rodokot[2] = new Stazysta(2100, 5);
    rodokot[3] = new Kierownik(4100, 3);

    for (auto &&i : rodokot)
    {
        i->przyznaj_bonus(200);
        std::cout << i->getPensja() << std::endl;
    }
